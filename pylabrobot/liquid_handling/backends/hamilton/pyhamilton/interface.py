...
class HamiltonResponse:
    ...
    def _parse_return(self):
        ...
        parsed = [
            parse(
                ...
                ",".join([' ' if item == '' else item for item in block.split(',') ]))
            for block in blocks
        ]
        if not parsed:
            return None
    ...
    def raise_first_exception(self):
        ...
        if isHamiltonStepError:
            ...
            raise HamiltonStepError(json.loads(self.raw)['step-return1'] if 'step-return1' in self.raw else self.raw)
...
class HamiltonInterface:
    ...
    def __init__(self, address=None, port=None, debug=False, simluating=False, **kwargs):
